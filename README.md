...
In AVOE, the input PDB file is expected to contain both the receptor and the ligand in the same structure file. The `--ligand` and `--ligand_file` options refer to input targets for overlap calculation, but the specified PDB files are protein-ligand complex structures rather than ligand-only structures.
...
- `--ligand <complex_pdb_file or directory>`: Path to a single PDB file containing both the receptor and the ligand for overlap-volume calculation, or a directory containing such PDB files. If a directory is specified, all PDB files within it will be processed. The directory name will be used as the ligand name.
- `--ligand_file <input_list_file>`: Path to a text file listing PDB files or directories to be processed, one path per line. Each listed PDB file should contain both the receptor and the ligand for overlap-volume calculation. Each listed directory should contain such PDB files. Cannot be used with `--ligand`. If neither `--ligand` nor `--ligand_file` is specified, AVOE will search for PDB files in the current directory and its subdirectories.
...
Example 1: process all complex PDB files in a directory corresponding to one ligand series.

```bash
python avoe.py --ligand input/ligandA/ --vdw_file vdw_radii.txt --output_dir output_avoe --pseudo_particles 5000 --repeat 3 --num_cores 4 --verbose
```

Example 2: process multiple ligand-series directories listed in a text file.

If you have the following structure:

```text
input/ligandA/WT.pdb
input/ligandA/G810R.pdb
input/ligandB/WT.pdb
input/ligandB/G810R.pdb
```
...
```bash
python avoe.py --ligand_file ligand_dirs.txt --vdw_file vdw_radii.txt --output_dir output_avoe
```

In this usage, each PDB file is a protein-ligand complex structure containing both receptor and ligand, and the directory name is treated as the ligand name.

## 5. Input File Formats

### 5.1 PDB Files

AVOE accepts standard PDB (Protein Data Bank) format files as input. The input PDB file should contain both the receptor and the ligand in a single complex structure file. AVOE reads atom coordinates and element information from ATOM records and distinguishes receptor and ligand atoms using the chain IDs specified by `--receptor_chain` and `--ligand_chain`.
...
### 5.4 Input List File for `--ligand_file`

The input list file is used with the `--ligand_file` option. It is a text file in which each line specifies either:
- a path to a PDB file containing both receptor and ligand for overlap-volume calculation, or
- a path to a directory containing such PDB files.
...
In this example, each directory contains protein-ligand complex PDB files such as `WT.pdb` and `G810R.pdb`, and the directory name is used as the ligand name in the output.
...
- **PDB File Format:** Ensure input PDB files are standard complex structure files containing both receptor and ligand, with ATOM records correctly specifying atom coordinates and element information.
